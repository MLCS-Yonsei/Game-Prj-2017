...
from crnn_model import cnn_basenet_2d


class ShadowNet(cnn_basenet_2d.CNNBaseModel):
    """
    ...
    def __feature_sequence_extraction(self, inputdata):
        ...
        conv3 = self.conv2d(inputdata=conv2, out_channel=256, kernel_size=3, stride=1, use_bias=False, name='conv3')  # batch*8*25*256 #66*27*256
        ...
        conv6 = self.conv2d(inputdata=bn5, out_channel=512, kernel_size=3, stride=1, use_bias=False, name='conv6')  # batch*4*25*512 #16*27*512
        ...
        max_pool6 = self.maxpooling(inputdata=bn6, kernel_size=[4, 1], stride=[4, 1])  # batch*2*25*512 #4*27*512
        conv7 = self.conv2d(inputdata=max_pool6, out_channel=512, kernel_size=2, stride=[4, 1], use_bias=False, name='conv7')  # batch*1*25*512 #1*27*512
        relu7 = self.relu(conv7)  # batch*1*25*512
        #conv8 = self.conv2d(inputdata=relu7, out_channel=512, kernel_size=4, stride=[4,4], use_bias=False, name='conv8')  # batch*4*25*512 #16*27*512 
