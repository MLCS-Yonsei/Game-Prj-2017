...
class CNNBaseModel(metaclass=ABCMeta):
    ...
    @staticmethod
    def conv3d(inputdata, out_channel, kernel_size, stride=1, padding='SAME', w_init=None, b_init=None,
               nl=tf.identity, split=1, use_bias=True, data_format='NDHWC', name=None):
        """
        ...
        with tf.variable_scope(name):
            ...
            if isinstance(kernel_size, list):
                filter_shape = [kernel_size[0], kernel_size[1], kernel_size[2]] + [in_channel / split, out_channel]
            else:
                filter_shape = [1, kernel_size, kernel_size] + [in_channel / split, out_channel]

            if isinstance(stride, list):
                strides = [1, stride[0], stride[1], stride[2], 1] if data_format == 'NDHWC' else [1, 1, stride[0], stride[1]]
            else: 
                strides = [1, 1, stride, stride, 1] if data_format == 'NDHWC' else [1, 1, stride, stride]
            ...
            if split == 1:
                conv = tf.nn.conv3d(inputdata, w, strides, padding, data_format=data_format)
            else:
                inputs = tf.split(inputdata, split, channel_axis)
                kernels = tf.split(w, split, 3)
                outputs = [tf.nn.conv3d(i, k, strides, padding, data_format=data_format)
    ...
    @staticmethod
    def maxpooling(inputdata, kernel_size, stride=None, padding='VALID', data_format='NDHWC', name=None):
        """
        ...
        if isinstance(kernel_size, list):
            kernel = [1, kernel_size[0], kernel_size[1], kernel_size[2], 1] if data_format == 'NDHWC' else \
                [1, 1, kernel_size[0], kernel_size[1]]
        else:
            kernel = [1, 1, 2, kernel_size, 1] if data_format == 'NDHWC' else [1, 1, kernel_size, kernel_size]
        ...
        return tf.nn.max_pool3d(input=inputdata, ksize=kernel, strides=strides, padding=padding,
                              data_format=data_format, name=name)

    @staticmethod
    def avgpooling(inputdata, kernel_size, stride=None, padding='VALID', data_format='NDHWC', name=None):
        """
        ...
        kernel = [1, kernel_size, kernel_size, 1] if data_format == 'NDHWC' else [1, 1, kernel_size, kernel_size]

        strides = [1, stride, stride, 1] if data_format == 'NDHWC' else [1, 1, stride, stride]

        return tf.nn.avg_pool(value=inputdata, ksize=kernel, strides=strides, padding=padding,
                              data_format=data_format, name=name)

    @staticmethod
    def globalavgpooling(inputdata, data_format='NDHWC', name=None):
        """
        ...
        assert data_format in ['NDHWC', 'NCHW']

        axis = [1, 2] if data_format == 'NDHWC' else [2, 3]

        return tf.reduce_mean(input_tensor=inputdata, axis=axis, name=name)

    @staticmethod
    def layernorm(inputdata, epsilon=1e-5, use_bias=True, use_scale=True, data_format='NDHWC', name=None):
        """
        :param name:
        :param inputdata:
    ...
    @staticmethod
    def instancenorm(inputdata, epsilon=1e-5, data_format='NDHWC', use_affine=True, name=None):
        """
