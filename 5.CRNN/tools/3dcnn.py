...
for i in range (10):
    if z==0:
        raw_image = pipe.stdout.read(264*110*3)
        image = np.fromstring(raw_image, dtype='uint8')
        image = [image.reshape((264,110,3))]
        z+=1
    else:
        raw_image = pipe.stdout.read(264*110*3)
        cap = np.fromstring(raw_image, dtype='uint8')
        cap = cap.reshape((264,110,3))
...
def inference(boxes):
    ...
    in_filters = 3#dataconfig.num_props
    ...
    with tf.variable_scope('softmax_linear') as scope:
        dim = np.prod(prev_layer.get_shape().as_list()[1:])
        weights = _weight_variable('weights', [dim, 37])#dataconfig.num_classes])
        biases = _bias_variable('biases', [37])#[dataconfig.num_classes])
